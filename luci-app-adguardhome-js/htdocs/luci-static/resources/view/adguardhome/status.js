...
return view.extend({
	...
	render_status_table: function (status, agh_config) {
		...
		const listen_addresses = L.isObject(status.dns_addresses) ? status.dns_addresses.join(
			', ') : _('Not found');
		const bootstrap_dns = L.isObject(agh_config.dns.bootstrap_dns) ? agh_config.dns
			.bootstrap_dns.join(', ') : _('Not found');
		const upstream_dns = L.isObject(agh_config.dns.upstream_dns) ? agh_config.dns.upstream_dns
			.join(', ') : _('Not found');
		const fields = [
			...
			table.appendChild(
				E('tr', {
					...
					E('td', {
						'class': 'td left',
						'width': '33%'
					}, Object.keys(objects[i])[0]),
					E('td', {
						'class': 'td left'
					}, Object.values(objects[i])[0])
				])
			);
		}
		return table;
				...
				E('div', {
					...
					E('div', {
						...
						E('div', {
							...
							'Please open the Configuration section, and provide the credentials.'
							))
					])
				]),
			]);
